...
#else
#ifdef SUSSING2013
        fprintf(fout, "%lu %10lu\n",		 (unsigned long)(halos[i].npart), (long unsigned)getSussing2013ID(simu.isnap,j));
#else
...
#else
#ifdef SUSSING2013
  fprintf(fout, "%10lu",			          (long unsigned)getSussing2013ID(simu.isnap, j));
  if(halos[i].hostHalo < 0)
    fprintf(fout, "\t%10ld",            (long)0);
  else
    fprintf(fout, "\t%10lu",            (long unsigned)getSussing2013ID(simu.isnap, idx_inv(idx, numHalos, halos[i].hostHalo)));
#else
